...
3.0.0 - Details Below:
* ~~Refactor `mockCoach.verify(mockCoach)` to be `mockCoach.verifyUpTo(mock)`. Better clarification that these methods run several verify lambdas up to a certain mock, instead of a single when/verify lambda.~~ Done
...
* Reduce constructor from 16 mocks (48 parameters) to 8 mocks (24 parameters). Current numbers of parameters is causing strange visual effects and lag on Intellij.
* Re-introduce Builder to allow N number of mocks. Support up to n number of mocks. This change is needed, because of the lowering of constructor mocks. 
